...
export async function obtenerTablero() {
  const [[ventasHoy], [inventario], [compras], [cajas], [ultimasVentas], [cuentas]] = await Promise.all([
    ...
    baseDatos.query(`SELECT COALESCE(SUM(saldo_pendiente), 0) AS saldo, COALESCE(SUM(CASE WHEN fecha_vencimiento < CURRENT_DATE() THEN saldo_pendiente ELSE 0 END), 0) AS vencido FROM ventas WHERE estado = 'completada'`),
  ]);
  return { ventas_hoy: { operaciones: Number(ventasHoy[0].operaciones), total: Number(ventasHoy[0].total) }, inventario: { productos: Number(inventario[0].productos), bajo_minimo: Number(inventario[0].bajo_minimo), valor_costo: Number(inventario[0].valor_costo) }, compras: { pendientes: Number(compras[0].pendientes), demoradas: Number(compras[0].demoradas) }, cajas_abiertas: Number(cajas[0].abiertas), cuentas_clientes: { saldo: Number(cuentas[0].saldo), vencido: Number(cuentas[0].vencido) }, ultimas_ventas: ultimasVentas };
}
