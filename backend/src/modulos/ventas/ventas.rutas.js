...
import { abrirCaja, crearVenta, obtenerSesion, referenciasVenta } from './ventas.servicio.js';
...
rutasVentas.post('/caja/abrir', requerirPermiso('caja.abrir'), async (req, res) => { const v = esquemaAbrirCaja.safeParse(req.body); if (!v.success) return res.status(400).json({ mensaje: 'Monto inicial inválido' }); try { res.status(201).json({ dato: await abrirCaja(req.usuario.id, v.data.monto_inicial) }); } catch (error) { if (error.codigoPublico === 'CAJA_ABIERTA') return res.status(409).json({ mensaje: error.message }); throw error; } });
