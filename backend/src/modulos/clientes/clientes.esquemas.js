...
export const esquemaConsultaClientes = z.object({ buscar: z.string().trim().max(160).optional(), estado: z.enum(['todos', 'activos', 'inactivos']).default('activos'), cuenta: z.enum(['todas', 'deudores', 'vencidas']).default('todas'), pagina: z.coerce.number().int().positive().default(1), limite: z.coerce.number().int().min(1).max(100).default(25) });
