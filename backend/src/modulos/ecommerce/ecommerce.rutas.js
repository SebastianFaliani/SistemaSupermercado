...
rutasEcommerce.post('/admin/pedidos/:id/devoluciones', requerirPermiso('ecommerce.pagos'), ejecutar(async (s, r) => { const d = validar(esquemas.esquemaDevolucionPedido, s.body, r); if (d) r.status(201).json({ dato: await servicio.devolverItemsPedido(Number(s.params.id), s.usuario.id, d) }); }));
