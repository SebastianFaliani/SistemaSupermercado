...
function datosDesdeFormulario(formulario) {
  ...
  const numeroOpcional = (nombre) => dato.get(nombre) === '' ? null : Number(dato.get(nombre));
  return {
    nombre: dato.get('nombre'),
    codigo_interno: dato.get('codigo_interno') || null,
    codigos_barra: dato.get('codigos_barra').split(',').map((codigo) => codigo.trim()).filter(Boolean),
  ...
  return (
    <form className="formulario-producto" onSubmit={alGuardar}>
      <div className="campos-producto">
        <div className="campo campo--ancho"><label htmlFor="producto_nombre">Nombre</label><input id="producto_nombre" name="nombre" defaultValue={producto?.nombre ?? ''} minLength="2" maxLength="180" required /></div>
        <div className="campo"><label htmlFor="producto_codigo_barra">Código de barras</label><input id="producto_codigo_barra" name="codigos_barra" defaultValue={producto?.codigos_barra?.join(', ') ?? ''} minLength="4" required placeholder="Separar varios con comas" /></div>
        <div className="campo"><label htmlFor="producto_codigo_interno">Código interno</label><input id="producto_codigo_interno" name="codigo_interno" defaultValue={producto?.codigo_interno ?? ''} /></div>
        <div className="campo"><label htmlFor="producto_categoria">Categoría</label><select id="producto_categoria" name="categoria_id" required defaultValue={producto?.categoria_id ?? ''}><option value="" disabled>Seleccionar</option>{categorias.map((categoria) => <option key={categoria.id} value={categoria.id}>{categoria.nombre}</option>)}</select></div>
        <div className="campo"><label htmlFor="producto_marca">Marca</label><select id="producto_marca" name="marca_id" defaultValue={producto?.marca_id ?? ''}><option value="">Sin marca</option>{referencias.marcas.map((marca) => <option key={marca.id} value={marca.id}>{marca.nombre}</option>)}</select></div>
        <div className="campo"><label htmlFor="producto_unidad">Unidad de medida</label><select id="producto_unidad" name="unidad_medida_id" required defaultValue={producto?.unidad_medida_id ?? ''}><option value="" disabled>Seleccionar</option>{referencias.unidades_medida.map((unidad) => <option key={unidad.id} value={unidad.id}>{unidad.nombre} ({unidad.abreviatura})</option>)}</select></div>
      ...
      <div className="modal__acciones">
        ...
        <button className="boton">{producto ? 'Guardar cambios' : 'Crear producto'}</button>
      </div>
    </form>
  );
}
...
export function Catalogo({ token, permisos }) {
  ...
  async function crearCategoria(evento) {
    ...
    try {
      ...
      await solicitar('/api/catalogo/categorias', token, { method: 'POST', body: JSON.stringify({ nombre: formulario.get('nombre') }) });
      setModalActivo(null);
  ...
  async function guardarProducto(evento) {
    ...
    try {
      ...
      const ruta = productoEditado ? `/api/catalogo/productos/${productoEditado.id}` : '/api/catalogo/productos';
      await solicitar(ruta, token, { method: productoEditado ? 'PUT' : 'POST', body: JSON.stringify(datos) });
      setModalActivo(null);
  ...
  async function abrirEdicion(productoId) {
    try {
      const respuesta = await solicitar(`/api/catalogo/productos/${productoId}`, token);
      ...
      <div className="rejilla-catalogo">
        ...
        <article className="panel panel--productos">
          ...
          <div className="tabla-contenedor">
            <table><thead><tr><th></th><th>Producto</th><th>Código</th><th>Categoría</th><th>Precio</th>{puedeGestionar && <th></th>}</tr></thead>
              <tbody>{productos.map((producto) => <tr key={producto.id}>
                <td>{producto.imagen_url ? <img className="miniatura-producto" src={producto.imagen_url} alt="" /> : <span className="miniatura-vacia" />}</td>
                <td>{producto.nombre}</td><td>{producto.codigo_barra}</td><td>{producto.categoria}</td>
                <td>${Number(producto.precio_venta).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</td>
                {puedeGestionar && <td><button className="boton-tabla" onClick={() => abrirEdicion(producto.id)}>Editar</button></td>}
              </tr>)}</tbody>
            </table>
          </div>
          <div className="paginacion">
            <button disabled={pagina === 1} onClick={() => setPagina((valor) => valor - 1)}>Anterior</button>
            <button disabled={pagina >= paginas} onClick={() => setPagina((valor) => valor + 1)}>Siguiente</button>
          </div>
        </article>
      </div>

      <Modal abierto={modalActivo === 'producto'} titulo={productoEditado ? 'Editar producto' : 'Nuevo producto'} ancho="grande" alCerrar={() => { setModalActivo(null); setProductoEditado(null); }}>
        <FormularioProducto key={productoEditado?.id ?? 'nuevo'} producto={productoEditado} categorias={categorias} referencias={referencias} alGuardar={guardarProducto} alCancelar={() => { setModalActivo(null); setProductoEditado(null); }} />
      </Modal>
