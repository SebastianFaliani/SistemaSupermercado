...
function FormularioUsuario({ usuario, roles, alGuardar, alCancelar }) {
  return <form className="formulario-modal" onSubmit={alGuardar}>
    <div className="campos-producto">
      <div className="campo"><label htmlFor="usuario_nombres">Nombres</label><input id="usuario_nombres" name="nombres" defaultValue={usuario?.nombres ?? ''} minLength="2" maxLength="100" required /></div>
      <div className="campo"><label htmlFor="usuario_apellidos">Apellidos</label><input id="usuario_apellidos" name="apellidos" defaultValue={usuario?.apellidos ?? ''} minLength="2" maxLength="100" required /></div>
      <div className="campo"><label htmlFor="usuario_nombre">Usuario</label><input id="usuario_nombre" name="nombre_usuario" defaultValue={usuario?.nombre_usuario ?? ''} minLength="3" maxLength="60" pattern="[a-zA-Z0-9._-]+" required /></div>
      <div className="campo"><label htmlFor="usuario_rol">Rol</label><select id="usuario_rol" name="rol_id" defaultValue={usuario?.rol_id ?? ''} required><option value="" disabled>Seleccionar</option>{roles.map((rol) => <option key={rol.id} value={rol.id}>{rol.nombre}</option>)}</select></div>
...
export function Usuarios({ token, permisos }) {
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      const [respuestaUsuarios, respuestaRoles] = await Promise.all([
        solicitar(`/api/usuarios?${parametros}`, token), solicitar('/api/usuarios/roles', token),
      ]);
  ...
  async function guardar(evento) {
    ...
    const datos = {
      nombre_usuario: formulario.get('nombre_usuario'), nombres: formulario.get('nombres'),
      apellidos: formulario.get('apellidos'), numero_documento: formulario.get('numero_documento') || null,
      correo_electronico: formulario.get('correo_electronico') || null,
      telefono: formulario.get('telefono') || null, rol_id: Number(formulario.get('rol_id')),
      clave: formulario.get('clave'),
      ...(usuarioEditado ? { esta_activo: formulario.get('esta_activo') === 'on' } : {}),
    };
    try {
      await solicitar(usuarioEditado ? `/api/usuarios/${usuarioEditado.id}` : '/api/usuarios', token, {
        method: usuarioEditado ? 'PUT' : 'POST', body: JSON.stringify(datos),
      });
      setModalAbierto(false);
      setUsuarioEditado(undefined);
      setMensaje(usuarioEditado ? 'Usuario actualizado correctamente.' : 'Usuario creado correctamente.');
      await cargar();
    } catch (error) { setMensaje(error.message); }
  }
  ...
  return <section className="modulo">
    <div className="modulo__encabezado">
      <div><p className="etiqueta">SEGURIDAD</p><h2>Usuarios</h2></div>
      {puedeGestionar && <button className="boton" onClick={() => { setUsuarioEditado(undefined); setModalAbierto(true); }}>Nuevo usuario</button>}
    </div>
    <div className="barra-filtros" role="search">
      <input value={textoBusqueda} onChange={(evento) => setTextoBusqueda(evento.target.value)} placeholder="Buscar por nombre, usuario, documento o correo" />
      <select aria-label="Filtrar por estado" value={estado} onChange={(evento) => { setEstado(evento.target.value); setPagina(1); }}><option value="todos">Todos los estados</option><option value="activos">Activos</option><option value="inactivos">Inactivos</option></select>
    </div>
    <p className="filtro-activo">Mostrando {total.toLocaleString('es-AR')} usuarios{buscar ? ` para “${buscar}”` : ''}.</p>
    {mensaje && <p className="mensaje" role="status">{mensaje}</p>}
    <article className="panel">
      <div className="panel__encabezado"><h3>Personal con acceso</h3><span>Página {pagina} de {paginas}</span></div>
      <div className="tabla-contenedor"><table><thead><tr><th></th><th>Nombre</th><th>Usuario</th><th>Rol</th><th>Estado</th><th>Último acceso</th>{puedeGestionar && <th></th>}</tr></thead>
    ...
    <Modal abierto={modalAbierto} titulo={usuarioEditado ? 'Editar usuario' : 'Nuevo usuario'} ancho="grande" alCerrar={() => { setModalAbierto(false); setUsuarioEditado(undefined); }}>
      <FormularioUsuario key={usuarioEditado?.id ?? 'nuevo'} usuario={usuarioEditado} roles={roles} alGuardar={guardar} alCancelar={() => { setModalAbierto(false); setUsuarioEditado(undefined); }} />
    </Modal>
  </section>;
}
