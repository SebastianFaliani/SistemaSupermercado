...
export function Compras({ token, permisos }) {
  ...
  function agregar(producto) { setItems((actual) => { const existente = actual.find((item) => item.producto_id === producto.id); const restantes = actual.filter((item) => item.producto_id !== producto.id); if (existente) return [{ ...existente, cantidad: Number(existente.cantidad) + 1 }, ...restantes]; return [{ producto_id: producto.id, nombre: producto.nombre, codigo_barra: producto.codigo_barra, es_pesable: producto.es_pesable, cantidad: 1, costo_unitario: Number(producto.precio_costo) }, ...actual]; }); setBusquedaProducto(''); setResultadoActivo(-1); requestAnimationFrame(() => buscadorProductoRef.current?.focus()); }
  ...
  async function recibir() {
    setRecibiendo(true);
    try {
      const detalles = compraARecibir.detalles.filter((item) => Number(cantidadesRecepcion[item.producto_id]) > 0).map((item) => ({ producto_id: item.producto_id, cantidad: Number(cantidadesRecepcion[item.producto_id]) }));
      const respuesta = await pedir(`/api/compras/${compraARecibir.id}/recibir`, token, { method: 'POST', body: JSON.stringify({ detalles }) });
  ...
  async function prepararRecepcion(compra) { try { const respuesta = await pedir(`/api/compras/${compra.id}`, token); const pendientes = Object.fromEntries(respuesta.dato.detalles.map((item) => [item.producto_id, String(Number(item.cantidad) - Number(item.cantidad_recibida))])); setCantidadesRecepcion(pendientes); setCompraARecibir(respuesta.dato); } catch (error) { setMensaje(error.message); } }
  async function confirmarAccionCompra() { setRecibiendo(true); try { await pedir(`/api/compras/${accionCompra.compra.id}/${accionCompra.accion}`, token, { method: 'POST' }); setMensaje(accionCompra.accion === 'enviar' ? 'Orden enviada al proveedor.' : 'Orden cancelada correctamente.'); setAccionCompra(null); await cargar(); } catch (error) { setMensaje(error.message); setAccionCompra(null); } finally { setRecibiendo(false); } }
  ...
  return <section className="modulo">
    ...
    <div className="barra-filtros"><input value={texto} onChange={(evento) => setTexto(evento.target.value)} placeholder="Buscar por número o proveedor" /><select value={estado} onChange={(evento) => { setEstado(evento.target.value); setPagina(1); }}><option value="todos">Todos los estados</option><option value="borrador">Borradores</option><option value="enviada">Enviadas</option><option value="parcial">Recepción parcial</option><option value="recibida">Recibidas</option><option value="cancelada">Canceladas</option></select></div>
    <p className="filtro-activo">Mostrando {total} órdenes.</p>{mensaje && <p className="mensaje">{mensaje}</p>}
    <article className="panel"><div className="panel__encabezado"><h3>Compras</h3><span>Página {pagina} de {paginas}</span></div>{compras.length ? <div className="tabla-contenedor"><table><thead><tr><th>Número</th><th>Creación</th><th>Entrega esperada</th><th>Recepción</th><th>Proveedor</th><th>Productos</th><th>Total</th><th>Estado</th><th></th></tr></thead><tbody>{compras.map((compra) => <tr key={compra.id}><td>#{compra.id}</td><td>{formatearFecha(compra.fecha_creacion)}</td><td>{compra.fecha_esperada ? formatearFecha(compra.fecha_esperada) : '—'}</td><td>{compra.fecha_recepcion ? formatearFecha(compra.fecha_recepcion) : '—'}</td><td>{compra.proveedor}</td><td>{compra.productos}</td><td>${Number(compra.total).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</td><td><span className="estado-activo">{compra.estado}</span></td><td className="acciones-tabla"><button className="boton-tabla" onClick={() => abrirOrden(compra)}>{compra.estado === 'borrador' ? 'Editar' : 'Ver'}</button>{compra.estado === 'borrador' && permisos.includes('compras.gestionar') && <button className="boton-tabla" onClick={() => setAccionCompra({ compra, accion: 'enviar' })}>Enviar</button>}{['enviada', 'parcial'].includes(compra.estado) && permisos.includes('compras.gestionar') && <button className="boton-tabla" onClick={() => prepararRecepcion(compra)}>Recibir</button>}{['borrador', 'enviada'].includes(compra.estado) && permisos.includes('compras.gestionar') && <button className="boton-tabla" onClick={() => setAccionCompra({ compra, accion: 'cancelar' })}>Cancelar</button>}</td></tr>)}</tbody></table></div> : <p className="vacio">Todavía no hay órdenes de compra.</p>}<div className="paginacion"><button disabled={pagina === 1} onClick={() => setPagina((valor) => valor - 1)}>Anterior</button><button disabled={pagina >= paginas} onClick={() => setPagina((valor) => valor + 1)}>Siguiente</button></div></article>
    <Modal abierto={modal} titulo={ordenActual ? `${soloLectura ? 'Detalle' : 'Editar'} compra #${ordenActual.id}` : 'Nueva orden de compra'} ancho="grande" alCerrar={cerrarModal}><form className="formulario-modal" onSubmit={guardar}>
      ...
      {!soloLectura && <div className="buscador-productos-compra"><label htmlFor="buscar_producto_compra">Buscar productos</label><input ref={buscadorProductoRef} id="buscar_producto_compra" role="combobox" aria-expanded={resultados.length > 0} aria-controls="resultados-busqueda-compra" aria-activedescendant={resultadoActivo >= 0 ? `resultado-compra-${resultados[resultadoActivo]?.id}` : undefined} value={busquedaProducto} onChange={(evento) => { setBusquedaProducto(evento.target.value); setResultadoActivo(-1); }} onKeyDown={navegarProductos} placeholder="Escribí nombre o código de barras" autoComplete="off" /><small className="ayuda-buscador">↑/↓ para elegir · Enter para agregar</small>
        {resultados.length > 0 && <div id="resultados-busqueda-compra" role="listbox" className="resultados-productos">{resultados.map((producto, indice) => <button id={`resultado-compra-${producto.id}`} role="option" aria-selected={resultadoActivo === indice} type="button" key={producto.id} className={resultadoActivo === indice ? 'resultado-activo' : ''} onMouseEnter={() => setResultadoActivo(indice)} onClick={() => agregar(producto)}><span>{producto.nombre}</span><small>{producto.codigo_barra || 'Sin código'} · Costo ${Number(producto.precio_costo).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</small></button>)}</div>}
      </div>}
      {items.length ? <div className="tabla-contenedor tabla-items-compra"><table><thead><tr><th>Producto agregado</th><th>Cantidad</th>{soloLectura && <><th>Recibida</th><th>Pendiente</th></>}<th>Costo unitario</th><th>Subtotal</th>{!soloLectura && <th></th>}</tr></thead><tbody>{items.map((item) => <tr key={item.producto_id}><td>{item.nombre}<small className="dato-secundario">{item.codigo_barra || 'Sin código'}</small></td><td><input type="number" min={item.es_pesable ? '0.001' : '1'} step={item.es_pesable ? '0.001' : '1'} value={item.cantidad} disabled={soloLectura} onFocus={(evento) => evento.currentTarget.select()} onChange={(evento) => cambiar(item.producto_id, 'cantidad', evento.target.value)} required /></td>{soloLectura && <><td>{Number(item.cantidad_recibida).toLocaleString('es-AR')}</td><td>{(Number(item.cantidad) - Number(item.cantidad_recibida)).toLocaleString('es-AR')}</td></>}<td><input type="number" min="0" step="0.01" value={item.costo_unitario} disabled={soloLectura} onFocus={(evento) => evento.currentTarget.select()} onChange={(evento) => cambiar(item.producto_id, 'costo_unitario', evento.target.value)} required /></td><td>${(Number(item.cantidad) * Number(item.costo_unitario)).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</td>{!soloLectura && <td><button type="button" className="boton-tabla" onClick={() => setItems((actual) => actual.filter((otro) => otro.producto_id !== item.producto_id))}>Quitar</button></td>}</tr>)}</tbody></table></div> : <p className="vacio">Buscá y agregá al menos un producto.</p>}
    ...
    <Modal abierto={Boolean(compraARecibir)} titulo="Registrar recepción" ancho="grande" alCerrar={() => { if (!recibiendo) setCompraARecibir(null); }}>
      {compraARecibir && <div className="confirmacion-modal"><p>Ingresá las cantidades recibidas de la compra <strong>#{compraARecibir.id}</strong>. Podés dejar productos en cero para recibirlos más adelante.</p><div className="tabla-contenedor tabla-items-compra"><table><thead><tr><th>Producto</th><th>Pedido</th><th>Ya recibido</th><th>Pendiente</th><th>Recibir ahora</th></tr></thead><tbody>{compraARecibir.detalles.map((item) => { const pendiente = Number(item.cantidad) - Number(item.cantidad_recibida); return <tr key={item.producto_id}><td>{item.nombre}</td><td>{Number(item.cantidad).toLocaleString('es-AR')}</td><td>{Number(item.cantidad_recibida).toLocaleString('es-AR')}</td><td>{pendiente.toLocaleString('es-AR')}</td><td><input type="number" min="0" max={pendiente} step={item.es_pesable ? '0.001' : '1'} value={cantidadesRecepcion[item.producto_id] ?? 0} onFocus={(evento) => evento.currentTarget.select()} onChange={(evento) => setCantidadesRecepcion((actual) => ({ ...actual, [item.producto_id]: evento.target.value }))} /></td></tr>; })}</tbody></table></div><div className="modal__acciones"><button type="button" className="boton boton--secundario" disabled={recibiendo} onClick={() => setCompraARecibir(null)}>Cancelar</button><button type="button" className="boton" disabled={recibiendo || !Object.values(cantidadesRecepcion).some((cantidad) => Number(cantidad) > 0)} onClick={recibir}>{recibiendo ? 'Recibiendo…' : 'Registrar recepción'}</button></div></div>}
    </Modal>
    <Modal abierto={Boolean(accionCompra)} titulo={accionCompra?.accion === 'enviar' ? 'Enviar orden' : 'Cancelar orden'} alCerrar={() => { if (!recibiendo) setAccionCompra(null); }}>{accionCompra && <div className="confirmacion-modal"><p>{accionCompra.accion === 'enviar' ? <>La compra <strong>#{accionCompra.compra.id}</strong> quedará enviada y habilitada para recibir mercadería.</> : <>La compra <strong>#{accionCompra.compra.id}</strong> será cancelada y no podrá recibirse.</>}</p><div className="modal__acciones"><button className="boton boton--secundario" disabled={recibiendo} onClick={() => setAccionCompra(null)}>Volver</button><button className="boton" disabled={recibiendo} onClick={confirmarAccionCompra}>{recibiendo ? 'Procesando…' : accionCompra.accion === 'enviar' ? 'Confirmar envío' : 'Confirmar cancelación'}</button></div></div>}</Modal>
  </section>;
}
